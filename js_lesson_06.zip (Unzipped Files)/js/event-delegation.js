...
function itemDone(e) {                       // Declare function
    ...
    /*
    The book used the following code - but it has two shortcomings
    - Clicking between list items would be remove the whose list
...
if (target.nodeName.toLowerCase() == "a") { // If user clicked on an a element
    elListItem = target.parentNode;         // Get its ul element
    elList = elListItem.parentNode;         // Get its li element
    elList.removeChild(elListItem);         // Remove list item from list
}
if (target.nodeName.toLowerCase() == "em") { // If user clicked on an a element
    elListItem = target.parentNode.parentNode;  // Get its ul element
    elList = elListItem.parentNode;            // Get its li element
    elList.removeChild(elListItem);            // Remove list item from list
}
...
if (el.addEventListener){                               // If event listeners work
    ...
    }, false);                                    // Using bubbling phase for flow
} else {                                               // Otherwise
  el.attachEvent('onclick', function (e){              // Use old IE model: onclick
      itemDone(e)                                      // Called itemDone
  });
}